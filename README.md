...
This project has been carried out by a group of Physics of Data students as final project of the course Laboratory of Computational Physics - Mod. B, held by Prof. Marco Baiesi. The subject of the project and the supervision of the work has been provided by Prof. Carlo Albert. 
...
$$\frac{\dot{S}(t)}{S(t)} = - h(\vec{x}(t))$$  
*(2)*

where $h(\vec{x})$ is a generic function of the dynamic variables. Since $S(t)$ is a (sort of) cumulative distribution function then $\dot{S}(t)$ is a probability distribution, and so $t \sim -\dot{S}(t)$, with $t \geq 0$, represents, in theory, a possible lifespan of the cell. This is a key concept because, for example, by collecting data samples of the microbes lifespans we can infer the parameters that describe $\dot{S}(t)$, which are also the parameters that describe $\vec{x}(t)$, allowing us to validate our models. <br>
...
This first model is very basic and aims to get the essential features that a good model should embed to correctly describe the phenomenon we are facing. This model is single-traited, namely
...
$$S(t) = \exp \Bigg(\omega_2 t \Big(\frac{u}{v} - 1\Big) - \frac{\omega_2 (m_0 + u)}{\omega_1 v} \Big(e^{\omega_1 t} - 1\Big)\Bigg)$$  
*(8)*
...
$$h(m) = \begin{cases} 0 \qquad \qquad m < u \\
...
$$\frac{\dot{S}(t)}{S(t)} = - h(m,p)$$  
*(14)*

with

$$h(m,p) = \begin{cases}0 \qquad \qquad \text{for} \quad p < u \\ 
\omega_2 \frac{p + v}{u + v} \qquad \text{for} \quad p \geq u\end{cases}$$  
*(15)*
...
$$p(t) = m_0 (e ^ {\omega_1 t} - 1)$$  
*(18)*

Thanks to eq. (18) we can look at eq. (15) and define

$$\hat{t} \equiv \frac{1}{\omega_1} \log \Big(\frac{u}{m_0} + 1\Big)$$  
*(19)*
...
$$S(t) = \exp \bigg(- \frac{\omega_2 m_0}{\omega_1 (u + v)} \Big[e ^ {\omega _1 t} - e ^ {\omega _1 \hat{t}} + \omega_1 (t - \hat{t}) \Big(\frac{v}{m_0} - 1\Big)\Big]\bigg) \qquad t \geq \hat{t}$$  
*(20b)*
...
This last model is the more advanced one as it introduces more elements of stochasticity aiming at catching more accurately the observed behaviour of microbe cells. The first main innovation that this model embeds is the presence of the growth rate as dynamic variable, thus
...
\int \delta(m - k m') f_{\beta}(k)  dk = f_{\Gamma}(\alpha) \delta(p) f_{\beta}\Big(\frac{m}{m'}\Big) \frac{1}{m'}$$  
*(22)*
...
$$p(t) = m_0 \big(e ^ {\alpha t} - 1\big)$$  
*(24)*

As before, by looking at eq. (15), we can define

$$\hat{t} \equiv \frac{1}{\alpha} \log\Big(\frac{u}{m_0} + 1\Big)$$  
*(25)*
...
$$S(t) = \exp \bigg(\frac{\omega_2}{(u+v)} [- \frac{m_0}{\alpha} \big(e ^ {\alpha t}-e^{\alpha \hat{t}}) + m_0 \big (t-\hat{t}) - v \big(t-\hat{t})]\bigg) \qquad t \geq \hat{t}$$  
*(26b)*
...
-\infty \quad \text{otherwise}\end{cases} \qquad \text{with} \quad A \subseteq \mathbb{R}^d$$
